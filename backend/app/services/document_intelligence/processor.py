...
from backend.app.services.document_intelligence.models import (
    FigureInfo,
    OCRElement,
    OCRLine,
    TableInfo,
)
from backend.app.services.document_intelligence.text_processing import (
    normalize_text,
)
from backend.app.services.document_intelligence.utils import (
    get_words,
    print_page_info,
)
...
class DocumentProcessor:
    ...
    def analyze_paragraphs(
        self, filename: str, preserve_linebreaks: bool = False
    ) -> list[str]:
        """
        Analyze document and group OCR lines into paragraphs with special handling for resume formatting.
        """
        ocr_lines = self.analyze_read(filename)
        paragraphs: list[str] = []

        def get_vertical_gap(curr_line: OCRLine, prev_line: OCRLine) -> float:
            """Calculate vertical gap between two lines."""
            curr_y = min(
                curr_line.polygon[i]
                for i in range(1, len(curr_line.polygon), 2)
            )
            prev_y = max(
                prev_line.polygon[i]
                for i in range(1, len(prev_line.polygon), 2)
            )
            return curr_y - prev_y

        def should_merge_with_previous(
            text: str, prev_text: str, gap: float
        ) -> bool:
            """Determine if current line should be merged with previous."""
            # Keep headers separate
            if prev_text.lower() in {
                "summary",
                "experience",
                "education",
                "skills",
            ...
            # Keep location and date lines separate
            if any(
                word in text.lower()
                for word in ["present", "2021", "2022", "2023", "united"]
            ):
                return False

            # Check if previous line ends with hyphen or comma, indicating continuation
            if prev_text.rstrip().endswith(("-", ",")):
                return True

            # If gap is small and previous line doesn't end with period, likely same paragraph
            if gap < 15 and not prev_text.rstrip().endswith("."):
                return True
        ...
        for i, line in enumerate(ocr_lines):
            ...
            gap = get_vertical_gap(line, ocr_lines[i - 1]) if i > 0 else 0
            prev_text = current_paragraph[-1]

            if should_merge_with_previous(text, prev_text, gap):
                if text.startswith("·"):
                    current_paragraph.append(text)  # Keep bullet on new line
                else:
                    # Merge with previous line
                    current_paragraph[-1] = f"{prev_text} {text}"
            else:
                # Start new paragraph
                if current_paragraph:
                    paragraphs.append(" ".join(current_paragraph))
    ...
    def _print_ocr_summary(
        ...
        print(f"Total words processed: {stats['total_words']}")
        print(
            f"Words filtered (confidence < {self.config.min_confidence}): {stats['filtered_words']}"
        )
        print(f"Lines extracted: {total_lines}")
        print("----------------------------------------")

    def _process_figure(self, figure: DocumentFigure) -> Optional[FigureInfo]:
        ...
        for word in words:
            ...
            if content:
                ...
                if (
                    ...
                    print(
                        f"......Filtered low confidence word: '{word.content}' ({confidence:.2f})"
                    )
        ...
        for line in page.lines:
            ...
            for word in words:
                if word.confidence >= self.config.min_confidence:
                    cleaned_content = (
                        normalize_text(word.content, preserve_newlines=False)
                        if self.config.clean_text
                        else word.content
                    )
    ...
    def _process_table(
        self, table: DocumentTable, page_number: int
    ) -> TableInfo:
        ...
        table_polygon = (
            table.bounding_regions[0].polygon if table.bounding_regions else []
        )
    ...
    def _process_word(self, word: DocumentWord) -> tuple[Optional[str], float]:
        ...
        return (
            (content.strip(), word.confidence)
            if content.strip()
            else (None, word.confidence)
        )
